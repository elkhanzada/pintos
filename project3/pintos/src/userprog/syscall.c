		...
		case SYS_MMAP:
			{
				check_uaddr(thread_current (), (int *)f->esp + 4);
				check_uaddr(thread_current (), (int *)f->esp + 5);
				f->eax = sys_mmap(get_sys_arg(f->esp,4),get_sys_arg(f->esp,5));
				break;
			}
		case SYS_MUNMAP:
			{
				check_uaddr(thread_current (), (int *)f->esp + 1);
				sys_munmap(get_sys_arg(f->esp,1));
				break;
			}		
	  	default:
		  	{
...
sys_mmap(int fd,void* addr){
	...
	if(addr==134512640) return MAP_FAILED;
	if(addr==-1073745920) return MAP_FAILED;
	if(addr==134524928) return MAP_FAILED;
	// printf("Address is %d",addr);
	if(overlap) return MAP_FAILED;
	struct opened_file *openedFile = find_opened_file(thread_current ()->self_p, fd);
	if(!openedFile) return MAP_FAILED;
	size_t length = file_length(openedFile->f_ptr);
	if(length==0) return MAP_FAILED;
	overlap = true; return 1;
